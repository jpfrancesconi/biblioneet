...
class EquipoClasificacionDAO extends GenericDAO {
    /**
   * @var const TABLE_NAME
   *  Name of the db table related with the entity
   */
  private const TABLE_NAME = 'io_equipo_clasificacion';
  /**
   * @var const TABLE_ALIAS
   *  Table alias used by the differents querys
   */
  private const TABLE_ALIAS = 'ec';
  /**
   * @var const TABLE_FIELD
   *  Name of the db table field used when the user send a search
   */
  private const TABLE_FIELD = 'clasificacion';

  /**
   * To get multiple Tipos de clasificación de eequipo records.
   *
   * @param array $header
  ...
  public static function getAll($header, $search_key = NULL ,$limit = NULL) {
    ...
    foreach ($result as $key => $row) {
      $entityDTO = self::getEquipoClasificacionDTOFromRecord($row);
  ...
  public static function load($id) {
    ...
    $entityDTO = self::getEquipoClasificacionDTOFromRecord($result);
  ...
  /**
   * Get the list of equipo clasificacion in the select format
   */
  public static function getEquipoClasificacionSelectFormat() {
    $select_options = parent::getListSelectFormat(self::TABLE_NAME, 'clasificacion', TRUE);

    return $select_options;
  }

   /**
   * Create an EquipoClasificacionDTO from stdClass from DB Record
   *
   * @param stdClass $row
   *   stdClass DB record
   * @return EquipoClasificacionDTO $equipoClasificacionDTO
   *   DTO object
   */
  private static function getEquipoClasificacionDTOFromRecord($row) {
    $equipoClasificacionDTO = new EquipoClasificacionDTO();
    $usuAltaDTO = new UsuarioDTO();
    $usuModDTO = new UsuarioDTO();

    $equipoClasificacionDTO->setId($row->id);
    $equipoClasificacionDTO->setClasificacion($row->clasificacion);
    $equipoClasificacionDTO->setActivo($row->activo);

    $usuAltaDTO->setUid($row->usuario_alta_uid);
    $usuAltaDTO->setUsername($row->usuario_alta);
    $equipoClasificacionDTO->setUsuarioAlta($usuAltaDTO);

    $equipoClasificacionDTO->setFechaAlta($row->fecha_alta);

    $usuModDTO->setUid($row->usuario_mod_uid);
    $usuModDTO->setUsername($row->usuario_mod);
    $equipoClasificacionDTO->setUsuarioMod($usuModDTO);

    $equipoClasificacionDTO->setFechaMod($row->fecha_mod);

    return $equipoClasificacionDTO;
  }
}
