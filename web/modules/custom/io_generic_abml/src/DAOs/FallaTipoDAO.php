...
class FallaTipoDAO extends GenericDAO {
   ...
   */
  public static function getAll($header, $equipo_tipo_id, $search_key = NULL ,$limit = NULL) {
    $limit = 15;
    $query = \Drupal::database()->select(self::TABLE_NAME, self::TABLE_ALIAS)
      ->fields(self::TABLE_ALIAS, ['id', self::TABLE_FIELD, 'activo', 'tipo_equipo_id', 'fecha_alta', 'fecha_mod']);
    // Add the audit fields to the query.
    $query =  parent::addAuditFields($query, self::TABLE_ALIAS);
    $query->condition(self::TABLE_ALIAS. '.tipo_equipo_id', $equipo_tipo_id, '=');
    ...
    foreach ($result as $key => $row) {
      $entityDTO = self::getFallaTipoDTOFromRecord($row);
  ...
  public static function load($id) {
    ...
    $entityDTO = self::getFallaTipoDTOFromRecord($result);
  ...
  public static function repeated($name) {
   ...
   /**
   * Create an FallaTipoDTO from stdClass from DB Record
   *
   * @param stdClass $row
   *   stdClass DB record
   * @return FallaTipoDTO $fallaTipoDTO
   *   DTO object
   */
  private static function getFallaTipoDTOFromRecord($row) {
    $fallaTipoDTO = new FallaTipoDTO();
    $equipoTipoDTO = new EquipoTipoDTO();
    $usuAltaDTO = new UsuarioDTO();
    $usuModDTO = new UsuarioDTO();

    $fallaTipoDTO->setId($row->id);
    $fallaTipoDTO->setFalla($row->falla);
    $fallaTipoDTO->setActivo($row->activo);

    $equipoTipoDTO = EquipoTiposDAO::load($row->tipo_equipo_id);
    $fallaTipoDTO->setTipoEquipo($equipoTipoDTO);

    $usuAltaDTO->setUid($row->usuario_alta_uid);
    $usuAltaDTO->setUsername($row->usuario_alta);
    $fallaTipoDTO->setUsuarioAlta($usuAltaDTO);

    $fallaTipoDTO->setFechaAlta($row->fecha_alta);

    $usuModDTO->setUid($row->usuario_mod_uid);
    $usuModDTO->setUsername($row->usuario_mod);
    $fallaTipoDTO->setUsuarioMod($usuModDTO);

    $fallaTipoDTO->setFechaMod($row->fecha_mod);
