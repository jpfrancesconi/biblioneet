...
class RegimenTipoDAO extends GenericDAO {
  ...
  public static function getAll($header, $search_key = NULL ,$limit = NULL) {
    ...
    foreach ($result as $key => $row) {
      $entityDTO = self::getRegimenTipoDTOFromRecord($row);
  ...
  public static function load($id) {
    ...
    $entityDTO = self::getRegimenTipoDTOFromRecord($result);

    return $entityDTO;
  }

  /**
   * To check if a tipo de regimen is valid.
   *
   * @param int $id
   *   The tipo de regimen ID.
   */
  ...
  /**
   * Edit an existing tipo de regimen
   *
   * @param int $id
   *   The tipo de regimen ID.
   * @param array $fields
   ...
   * @return int
   *  Id of the tipo de regimen added.
   */
  ...
  /**
   * To delete a specific tipo de regimen record.
   *
   * @param int $id
   *   The tipo de regimen ID.
   */
  ...
  /**
   * To check if a  tipo de regimen name exists in the db.
   *
   * @param string $name
   *   The tipo de regimen name.
   */
  ...
  /**
   * Get the list of tipos de regimen in the select format
   */
  public static function getRegiemnTipoListSelectFormat() {
    $select_options = parent::getListSelectFormat(self::TABLE_NAME, self::TABLE_FIELD);

    return $select_options;
  }
   /**
   * Create an RegimenTipoDTO from stdClass from DB Record
   *
   * @param stdClass $row
   *   stdClass DB record
   * @return RegimenTipoDTO $regimenTipoDTO
   *   DTO object
   */
  private static function getRegimenTipoDTOFromRecord($row) {
    $regimenTipoDTO = new RegimenTipoDTO();
    $usuAltaDTO = new UsuarioDTO();
    $usuModDTO = new UsuarioDTO();

    $regimenTipoDTO->setId($row->id);
    $regimenTipoDTO->setTipoRegimen($row->tipo_regimen);
    $regimenTipoDTO->setActivo($row->activo);

    $usuAltaDTO->setUid($row->usuario_alta_uid);
    $usuAltaDTO->setUsername($row->usuario_alta);
    $regimenTipoDTO->setUsuarioAlta($usuAltaDTO);

    $regimenTipoDTO->setFechaAlta($row->fecha_alta);

    $usuModDTO->setUid($row->usuario_mod_uid);
    $usuModDTO->setUsername($row->usuario_mod);
    $regimenTipoDTO->setUsuarioMod($usuModDTO);

    $regimenTipoDTO->setFechaMod($row->fecha_mod);

    return $regimenTipoDTO;
  }
}
