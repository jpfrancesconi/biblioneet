...
class ActividadClasificacionDAO extends GenericDAO {
    /**
   * @var const TABLE_NAME
   *  Name of the db table related with the entity
   */
  private const TABLE_NAME = 'io_actividad_clasificacion';
  /**
   * @var const TABLE_ALIAS
   *  Table alias used by the differents querys
   */
  private const TABLE_ALIAS = 'ac';
  /**
  ...
  public static function getAll($header, $search_key = NULL ,$limit = NULL) {
    ...
    foreach ($result as $key => $row) {
      $entityDTO = self::getActividadClasificacionDTOFromRecord($row);
  ...
  public static function load($id) {
    ...
    $entityDTO = self::getActividadClasificacionDTOFromRecord($result);
  ...
  public static function repeated($name) {
   ...
   /**
   * Create an ActividadClasificacionDTO from stdClass from DB Record
   *
   * @param stdClass $row
   *   stdClass DB record
   * @return ActividadClasificacionDTO $actividadClasificacionDTO
   *   DTO object
   */
  private static function getActividadClasificacionDTOFromRecord($row) {
    $actividadClasificacionDTO = new ActividadClasificacionDTO();
    $usuAltaDTO = new UsuarioDTO();
    $usuModDTO = new UsuarioDTO();

    $actividadClasificacionDTO->setId($row->id);
    $actividadClasificacionDTO->setTipo($row->tipo);
    $actividadClasificacionDTO->setActivo($row->activo);

    $usuAltaDTO->setUid($row->usuario_alta_uid);
    $usuAltaDTO->setUsername($row->usuario_alta);
    $actividadClasificacionDTO->setUsuarioAlta($usuAltaDTO);

    $actividadClasificacionDTO->setFechaAlta($row->fecha_alta);

    $usuModDTO->setUid($row->usuario_mod_uid);
    $usuModDTO->setUsername($row->usuario_mod);
    $actividadClasificacionDTO->setUsuarioMod($usuModDTO);

    $actividadClasificacionDTO->setFechaMod($row->fecha_mod);

    return $actividadClasificacionDTO;
  }
}
