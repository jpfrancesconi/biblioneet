...
class PrioridadDAO extends GenericDAO {
    /**
   * @var const TABLE_NAME
   *  Name of the db table related with the entity
   */
  private const TABLE_NAME = 'io_prioridad';
  /**
   ...
   */
  private const TABLE_FIELD = 'prioridad';
  ...
  public static function getAll($header, $search_key = NULL ,$limit = NULL) {
    ...
    foreach ($result as $key => $row) {
      $entityDTO = self::getPrioridadDTOFromRecord($row);
  ...
  public static function load($id) {
    ...
    $prioridadDTO = self::getPrioridadDTOFromRecord($result);
  ...
  public static function repeated($name) {
   ...
   /**
   * Create an PrioridaDTO from stdClass from DB Record
   *
   * @param stdClass $row
   *   stdClass DB record
   * @return PrioridadDTO $prioridadDTO
   *   DTO object
   */
  private static function getPrioridadDTOFromRecord($row) {
    $prioridadDTO = new PrioridadDTO();
    $usuAltaDTO = new UsuarioDTO();
    $usuModDTO = new UsuarioDTO();

    $prioridadDTO->setId($row->id);
    $prioridadDTO->setPrioridad($row->prioridad);
    $prioridadDTO->setActivo($row->activo);

    $usuAltaDTO->setUid($row->usuario_alta_uid);
    $usuAltaDTO->setUsername($row->usuario_alta);
    $prioridadDTO->setUsuarioAlta($usuAltaDTO);

    $prioridadDTO->setFechaAlta($row->fecha_alta);

    $usuModDTO->setUid($row->usuario_mod_uid);
    $usuModDTO->setUsername($row->usuario_mod);
    $prioridadDTO->setUsuarioMod($usuModDTO);

    $prioridadDTO->setFechaMod($row->fecha_mod);
