...
class UnidadMedidaDAO extends GenericDAO {
  ...
  public static function getAll($header, $search_key = NULL ,$limit = NULL) {
    ...
    $query = \Drupal::database()->select(self::TABLE_NAME, self::TABLE_ALIAS)
      ->fields(self::TABLE_ALIAS, ['id', self::TABLE_FIELD , 'fecha_alta', 'fecha_mod']);
    ...
    foreach ($result as $key => $row) {
      $entityDTO = self::getUnidadMedidaDTOFromRecord($row);
  ...
  public static function load($id) {
    ...
    $prioridadDTO = self::getUnidadMedidaDTOFromRecord($result);
  ...
  /**
   * Get the list of unidad de medida in the select format
   */
  public static function getUnidadMedidaSelectFormat() {
  ...
  /**
   * Create an UnidadMedidaDTO from stdClass from DB Record
   *
   * @param stdClass $row
   *   stdClass DB record
   * @return UnidadMedidaDTO $unidadMedidaDTO
   *   DTO object
   */
  private static function getUnidadMedidaDTOFromRecord($row) {
    $unidadMedidaDTO = new UnidadMedidaDTO();
    $usuAltaDTO = new UsuarioDTO();
    $usuModDTO = new UsuarioDTO();

    $unidadMedidaDTO->setId($row->id);
    $unidadMedidaDTO->setUnidadMedida($row->unidad_medida);

    $usuAltaDTO->setUid($row->usuario_alta_uid);
    $usuAltaDTO->setUsername($row->usuario_alta);
    $unidadMedidaDTO->setUsuarioAlta($usuAltaDTO);

    $unidadMedidaDTO->setFechaAlta($row->fecha_alta);

    $usuModDTO->setUid($row->usuario_mod_uid);
    $usuModDTO->setUsername($row->usuario_mod);
    $unidadMedidaDTO->setUsuarioMod($usuModDTO);

    $unidadMedidaDTO->setFechaMod($row->fecha_mod);
