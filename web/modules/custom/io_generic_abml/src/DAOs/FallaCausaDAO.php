...
class FallaCausaDAO extends GenericDAO {
  ...
  public static function getAll($header, $search_key = NULL ,$limit = NULL) {
    ...
    foreach ($result as $key => $row) {
      $entityDTO = self::getFallaCausaDTOFromRecord($row);
  ...
  public static function load($id) {
    ...
    $entityDTO = self::getFallaCausaDTOFromRecord($result);

    return $entityDTO;
  }

  /**
   * To check if a causa de la falla is valid.
   *
   * @param int $id
   *   The causa de la falla ID.
   */
  ...
  /**
   * Edit an existing causa de la falla
   *
   * @param int $id
   *   The causa de la falla ID.
   * @param array $fields
   ...
   * @return int
   *  Id of the causa de la falla added.
   */
  ...
  /**
   * To delete a specific causa de la falla record.
   *
   * @param int $id
   *   The causa de la falla ID.
   */
  ...
  /**
   * To check if a  causa de la falla name exists in the db.
   *
   * @param string $name
   *   The causa de la falla name.
   */
  public static function repeated($name) {
   ...
   /**
   * Create an FallaCausaDTO from stdClass from DB Record
   *
   * @param stdClass $row
   *   stdClass DB record
   * @return FallaCausaDTO $causaFallaDTO
   *   DTO object
   */
  private static function getFallaCausaDTOFromRecord($row) {
    $causaFallaDTO = new FallaCausaDTO();
    $usuAltaDTO = new UsuarioDTO();
    $usuModDTO = new UsuarioDTO();

    $causaFallaDTO->setId($row->id);
    $causaFallaDTO->setCausaFalla($row->causa);
    $causaFallaDTO->setActivo($row->activo);

    $usuAltaDTO->setUid($row->usuario_alta_uid);
    $usuAltaDTO->setUsername($row->usuario_alta);
    $causaFallaDTO->setUsuarioAlta($usuAltaDTO);

    $causaFallaDTO->setFechaAlta($row->fecha_alta);

    $usuModDTO->setUid($row->usuario_mod_uid);
    $usuModDTO->setUsername($row->usuario_mod);
    $causaFallaDTO->setUsuarioMod($usuModDTO);

    $causaFallaDTO->setFechaMod($row->fecha_mod);

    return $causaFallaDTO;
  }
}
