 ...
 * Entity Form.
 */
class EntityForm extends FormBase implements FormInterface {
  /**
  ...
  public function buildForm(array $form, FormStateInterface $form_state, $author = NULL) {
    if ($author) {
      ...
      $author = EntitiesDAO::load($author);
      $form['aid'] = [
        '#type' => 'hidden',
        '#value' => $author->id,
      ];
    ...
    $form['first_name'] = [
      ...
      '#required'       => TRUE,
      '#default_value'  => ($author) ? $author->first_name : '',
    ...
    $form['last_name'] = [
      ...
      '#required'       => TRUE,
      '#default_value'  => ($author) ? $author->last_name : '',
    ...
    $form['nationality'] = [
      ...
      '#default_value'  => ($author) ? $author->nationality : '',
    ];
    ...
    $form['description'] = [
      ...
      '#default_value'  => ($author) ? $author->description : '',
      '#required'       => FALSE,
    ];
    ...
    $form['upload']['picture'] = [
      ...
      '#default_value'      => ($author) ? [$author->picture] : '',
    ];
    ...
    $form['status'] = [
      ...
      '#default_value'  => ($author) ? $author->status : 1,
    ];
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    if (!empty($id) && EntitesDAO::exists($id)) {
      $author = EntitesDAO::load($id);
      if ($picture_fid) {
      ...
      $new_author_id = EntitesDAO::add($fields);
      if ($picture_fid) {
        $file = File::load($picture_fid);
        $file->setPermanent();
        $file->save();
