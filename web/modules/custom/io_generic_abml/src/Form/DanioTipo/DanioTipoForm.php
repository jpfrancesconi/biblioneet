...
class DanioTipoForm extends FormBase implements FormInterface {
  /**
   * ROUTE_TO_RETURN is the route to return at the end of the differntes form flows.
   */
  const ROUTE_TO_RETURN = 'io_generic_abml.danio_tipo.list';
  ...
  public function buildForm(array $form, FormStateInterface $form_state, $id = NULL) {
    ...
    $form['subtitle'] = [
      '#type'   => 'item',
      '#title'  => t('Desde esta ventana podrá dar de alta nuevos tipos de daño para que sean luego asociados a las actividades.'),
    ];
    ...
    $form['danio'] = [
      ...
      '#title'          => t('Daño'),
      '#required'       => TRUE,
      '#default_value'  => ($danioTipo) ? $danioTipo->getDanio() : '',
      '#description'    => t('Nombre del tipo de daño'),
      '#attributes' => [
        'autocomplete' => 'off',
      ],
    ];
    ...
    $form['activo'] = [
      ...
      '#title'          => t('Activo'),
      '#required'       => FALSE,
      '#default_value'  => ($danioTipo) ? $danioTipo->getActivo() : 1,
      '#description'    => t('Estado del tipo de daño'),
      '#attributes' => [
        'autocomplete' => 'off',
      ],
    ];
  ...
  public function commonSubmit(array &$form, FormStateInterface $form_state) {
    ...
    $fields = [
      'danio' => $form_state->getValue('danio'),
      'activo' => $form_state->getValue('activo'),
      'usuario_alta' => $user->id(),
    ];

    if (!empty($id) && DanioTipoDAO::exists($id)) {
      // Set Updated audit fields
      $fields['usuario_mod'] = $user->id();
      $fields['fecha_mod'] = date("Y-m-d h:m:s");
      DanioTipoDAO::update($id, $fields);
      $message = 'El tipo de daño [' . $form_state->getValue('danio') . '] ha sido actualizado correctamente.';
    } else {
      $new_record_id = DanioTipoDAO::add($fields);
      $message = 'El tipo de daño [' . $form_state->getValue('danio') . '] ah sido creado satisfactoriamente.';
    }
